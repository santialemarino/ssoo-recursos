...
| [Deadlock](deadlock/) *(sin publicar)* | Deadlock paso a paso: la espera que no es deadlock y el ciclo que sí, el ciclo que igual no traba porque hay dos instancias, las cuatro condiciones con lo que cuesta romper cada una, el estado seguro y el algoritmo del banquero, la detección con su víctima y su frecuencia, y el cierre —deadlock contra livelock, y qué estrategia le sirve a cada sistema—. Dieciséis ejemplos en escalera, agrupados en cinco tramos. |
| [Planificación de procesos](cpu-scheduling/) | El estudiante pinta el diagrama de Gantt —elige un recurso y marca quién lo ocupa en cada instante— y el recurso le corrige cada respuesta, mostrándole la correcta y por qué es esa. FIFO, SJF, HRRN, Round Robin, Virtual Round Robin, colas multinivel con feedback, hilos con y sin jacketing, dos procesadores y swap. Diecisiete ejemplos en escalera, y el último es reconocer el algoritmo mirando un diagrama ya resuelto. |
...
| [Sincronización](synchronization/) | Semáforos y secciones críticas paso a paso: la condición de carrera instrucción por instrucción, un intercalado que el estudiante elige, las soluciones por software y por hardware, el semáforo por dentro con su valor y su cola de bloqueados, y los usos —cinco impresoras, dos procesos que se pasan el turno, productor-consumidor con lista acotada e inversión de prioridades—. Dieciocho ejemplos en escalera, agrupados en cinco tramos. |
